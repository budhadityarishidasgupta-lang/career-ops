...
A unit file ships at `packaging/career-ops.service`. Copy + edit it:

```bash
sudo cp packaging/career-ops.service /etc/systemd/system/hireloom.service
...
cp packaging/com.hireloom.atelier.plist ~/Library/LaunchAgents/
...
> See `packaging/career-ops.service` as the reference.

### Windows — Task Scheduler

Create a Scheduled Task with trigger `At log on`, action `node
"C:\path\to\hireloom\dashboard-web\server.mjs"`. Set "Run whether user is
